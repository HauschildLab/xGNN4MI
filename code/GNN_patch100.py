...
class GCN_100(torch.nn.Module):
    def __init__(self, dataset, num_nodes, num_patches):
        super(GCN_100, self).__init__()
        self.num_nodes = num_nodes
        self.num_patches = num_patches
        
        # Adjust the FFN to match the input size for the first GCN layer
        self.ffn1 = nn.Sequential(
            nn.Linear(10,38400),  # Adjust the output size to match the input size for GCNConv
            nn.BatchNorm1d(38400),
